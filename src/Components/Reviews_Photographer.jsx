    ...
    const fetchReviews = async () => {
      try {
        const response = await axios.get(
          `http://localhost:3001/api/photographer/${id}/reviews`,
          {
            headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://localhost:3001/api/reviewsPhotographer",
        reviewDataToSend,
        {
          headers: {
            Authorization: `Bearer ${token}`,
...
export default Reviews_Photographer;
